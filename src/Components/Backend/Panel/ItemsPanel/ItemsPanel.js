...
const ItemsPanel = (properties) => {
	...
	return <>
		{'single' === design && undefined !== activeIndex && <>
			<h3 className='bplItemTitle'>{items?.[activeIndex]?.title}</h3>
		...
		{'all' === design && items.map((item, index) => {
			return <PanelBody key={index} className='bPlPanelBody itemPanelBody' title={item.title} initialOpen={0 !== index ? false : true}>
